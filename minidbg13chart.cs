...
class MiniDbg
{
    ...
    static Dictionary<ulong, string> chartNames = new Dictionary<ulong, string>();
    ...
    static int Main(string[] args)
    {
        ...
        foreach (string part in ctl[1].Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            {
                string[] kv2 = part.Substring(6).Split('=');
                chartNames[Convert.ToUInt64(kv2[0], 16)] = kv2.Length > 1 ? kv2[1] : ("chart_" + kv2[0]);
                continue;
            }
        ...
        while (!exited)
        {
            ...
            else if (code == LOAD_DLL_DEBUG_EVENT)
            {
                ...
                if (nm.ToLower().EndsWith("\\chart.dll"))
                {
                    IntPtr savedProc = hProc;
                    if (procHandles.ContainsKey(pid)) hProc = procHandles[pid];
                    L("chart.dll loaded in pid=" + pid + " base=0x" + baseDll.ToString("X"));
                    foreach (KeyValuePair<ulong, string> kv in chartNames)
                    {
                        ulong addr = baseDll + kv.Key;
                        byte[] orig = new byte[1]; IntPtr rd, wr;
                        if (ReadProcessMemory(hProc, new IntPtr((long)addr), orig, new IntPtr(1), out rd))
                        {
                            ...
                            if (WriteProcessMemory(hProc, new IntPtr((long)addr), cc, new IntPtr(1), out wr))
                            {
                                ...
                                bps[addr] = bi;
                                L("BP set " + kv.Value + " @ chart+0x" + kv.Key.ToString("X") + " (0x" + addr.ToString("X") + ")");
                            }
                        }
                    }
                    hProc = savedProc;
                ...
                if (bpHit)
                {
                    ...
                    try
                    {
                        ...
                        if (GetThreadContext(hT, ctxA))
                        {
                            ...
                            try { byte[] ctxdump = new byte[0x200]; for (int _i=0; _i<0x200; _i++) ctxdump[_i] = Marshal.ReadByte(ctxA, _i); L("CTXHEX " + BitConverter.ToString(ctxdump).Replace("-","")); } catch (Exception _e) { L("CTXDUMP_FAIL " + _e.Message); }
